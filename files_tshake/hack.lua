...
local function keko_tshake(data)
    ...
    if text and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
        if text:match("@(.*)") and database:get("keko:hack:tshake:" .. bot_id .. msg.sender_user_id_) then
            ...
            send(msg.chat_id_, msg.id_, 1, "📉┇ تم تغير قناة الاشتراك الاجباري اشترك بقناة السورس لتفعيل بوتك @disco3", 1, "html")
        ...
        if text == "تغير القناة" then
            ...
            send(msg.chat_id_,msg.id_,1,"🔖┇ارسل يوزر قناتك بعد رفع بوتك ادمن بالقناة\n!!",1,"html")
            return "keko"
        end
    end
end
